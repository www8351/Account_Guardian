...
5a-bis, ADDED 2026-07-30 by owner order, a dependency that would otherwise surface mid-matrix at A9. R1 was ruled as the pairwise-bound primitive and R2 absorbed with the CrashLoopWindowSeconds default moving from 60 to 300. All twelve a9_*.set vectors written in Stage 2 encode CrashLoopWindowSeconds=60, so every one of them is stale the moment the new default lands and must be regenerated. Regeneration is not sufficient on its own: a running terminal never enumerates files added to its data folder after it started, measured on 2026-07-30 when the vectors stayed invisible in the Load browser until an incidental restart made them appear. So Stage 5a must, in this order: implement the ruled primitive, regenerate all twelve vectors against the new default, and only then restart the terminal, folding the restart into the first A3 kill cycle rather than spending a separate one. Verify visibility in the Load browser before any A9 row runs.
