...
**Correction, 2026-08-08, second pass.** Numbered A6, not A5. Verified this session, against the object store directly, not against any report: `worktree-a9-phantom-audit` (tip `461c415`, last LEDGER-touching commit `0c1e0b3`) carries a FINAL owner ruling of 2026-08-05 on the A10 transition-table findings, applied to `docs/SPEC_v0.1.md` as amendment A5 in that same branch, three section 2 table edits unrelated to Phase 1: the boot-to-SYNCING From cell widened, the any-to-SAFE_HALT row split into a boot row and an any row, and the SAFE_HALT action cell corrected. That branch is not merged to main (main is `bb956b2`, LEDGER.md differs from the branch by 90 lines, `0c1e0b3` is not an ancestor of main). Taking A5 for this plan's amendment would collide the moment that branch merges. Every "A5" reference elsewhere in this document is A6.
