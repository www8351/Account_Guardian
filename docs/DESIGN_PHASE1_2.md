...
### Q6 clarification, drafted for ratification

Q6 (FINAL) states the locked window is judged by the snapshot, never live inputs. That rule presumes a snapshot exists to read. Item 3 above adds a fallback for the case Q6's own wording left silent: no valid snapshot available (file missing, corrupted, or login-mismatched — precisely F4's own defense scenario). Drafted wording, pending owner ratification:

> Q6 clarification (proposed by the executor, pending owner ratification): Q6's snapshot rule governs the locked window once a valid snapshot exists. It does not by itself state what governs the derived-breach comparison (SPEC 4.6) when no valid snapshot is available to read. In that specific case, the derived comparison falls back to the limit computed live from current inputs and the live-reconstructed base (SPEC 4.2/4.3), not to any cached or assumed value. This does not weaken Q6: a valid snapshot, once available, still governs unconditionally and always wins over live inputs, exactly as ruled. The fallback only ever activates in the absence of a snapshot to read, which is precisely the condition under which Q6's original rule has nothing to apply to. Consequence, stated plainly: this fallback is what makes file+GV deletion alone (inputs untouched) fully defended by derived history; it is also why file+GV deletion combined with input inflation remains an accepted residual gap (SPEC 4.6), since the fallback then reads the inflated input as if it were legitimate.

Recommendation: ratify as a clarification, not a new rule. It governs a case Q6's original wording left silent and changes no obligation Q6 already covers.

### Active-state limit inflation: ratchet defense, costed, not ruled

The executor's original dismissal of this scenario is withdrawn as reasoning: "nothing claims to prevent the account owner from loosening their own settings" inverts the product's premise. The threat model is the owner circumventing his own guardian under pressure; opening the properties dialog and raising `DailyLossPercent` before any loss takes seconds, touches no file, forges no clock, kills nothing, and is cheaper than every bypass F4 defends against.

**What is ratcheted:** the raw core inputs (`DailyLossPercent`, `DailyLossCurrency`), not the derived `limit_currency` directly. Within a single day, Base is invariant to everything (item 2's proof holds for all deal types, not only balance-type ones — Base is exactly the day-anchor balance from anchor to next anchor, full stop). So `percent_limit_currency = DailyLossPercent% × Base` moves only if the input moves; ratcheting the input is equivalent to ratcheting the derived value and composes cleanly with Q8's stricter-wins formula: ratchet each of `DailyLossPercent`-floor and `DailyLossCurrency`-floor independently toward stricter-only (smaller-only), then feed the ratcheted pair through the existing min-of-enabled-candidates rule. A ratcheted leg cannot be loosened by disabling it (setting it to 0) either — disabling is the most extreme form of loosening and must be blocked the same as any partial loosening.

**Persistence, home, and discipline:** a new file, `floor_<login>.dat`, not an extension of the lock state file. This mirrors Amendment 2a's own precedent exactly: SAFE_HALT got its own file instead of widening the charter-constrained lock state file's scope ("lock state only, nothing else, per charter"); the ratchet floor is not lock state either, and deserves the same separation. Same atomic-write path (tmp, `FileFlush`, `FileMove FILE_REWRITE`), same checksum reuse (`AgChecksum`), same never-loaded-never-written discipline via a third flag (`g_ag_floor_loaded`, mirroring `g_ag_halt_loaded` and `g_ag_state_loaded`). Fields: `floor_day_anchor`, `percent_floor`, `currency_floor`, checksum.

**GV-mirrored, and why more so than the lock:** yes, mirror to GV, refreshed every tick like `AG_LOCK`. The lock has three independent witnesses, one of which (broker history) cannot be forged locally. The ratchet floor has, at most, two — file and GV, both locally writable — because an input value is never a deal; it leaves no trace in broker history. There is no third leg possible for this artifact on this platform, so the two weak witnesses are the entire defense, making the redundancy between them more load-bearing here than it is for the lock, not less.

**Interaction with the breach snapshot:** none needed. The ratchet governs pre-breach ACTIVE; Q6's snapshot governs post-breach LOCKED; the two phases are disjoint. Since `limit_currency` is already computed from the ratcheted-floor inputs at all times during ACTIVE, `limit_snapshot` taken at the moment of breach is automatically the ratcheted value — it falls out of "snapshot whatever `limit_currency` currently is" with no extra obligation.

**Day-anchor release:** automatic, no deliberate act, mirroring unlock-by-expiry exactly as proposed. At each new day anchor, `percent_floor` and `currency_floor` re-baseline to whatever the live inputs currently read, and can only tighten further from that point for the rest of the new day.

**Residual, sized honestly:** this is friction, not prevention, and weaker friction than the lock's. Destroying both the floor file and the floor GV (no input inflation even required) resets the floor to the live input with nothing to catch it, because there is no history-backed third witness for an input value on this platform — a hard platform ceiling, not a design shortfall, the same class of ceiling already accepted for Q3/F2. Honestly sized: it raises the bar from one click (five seconds) to roughly three or four deliberate steps (open dialog, change value, clear the floor file, clear the floor GV, confirm) — real friction against an impulsive act, no friction at all against a determined, multi-step one. It does not claim to do more than that.

**Alternative, costed: log loudly and accept.** Cost: zero new files, zero new persisted state, zero new GV, zero new load/save/corruption code path, zero new acceptance rows. Benefit: a loud, auditable WARN line the instant `DailyLossPercent`/`DailyLossCurrency` is loosened while ACTIVE, distinct from the already-mandated while-LOCKED ALERT. Downside: does not stop the five-second bypass at all, only makes it visible afterward — the same detect-only posture already accepted for the trade-disallowed-states family (Q3/F2), extended to a new scenario rather than an unprecedented one.

**Recommendation (not a ruling):** the ratchet. The threat the owner named is impulsive, under pressure, seeking the fastest possible relief — exactly the failure mode the ratchet's extra deliberate step defeats, even though a determined multi-step actor still gets through the same way file+GV destruction already gets through elsewhere in this design. Design only; awaiting the owner's ruling before any obligation here is treated as binding.
