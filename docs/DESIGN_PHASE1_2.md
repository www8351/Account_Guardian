...
**LEDGER.md write freeze, LIFTED 2026-08-03 by merge `ce6964d`, kept here for the record.** Owner instruction (2026-07-30): no further LEDGER.md commits on `main` until the owner lifts it. Reason: `main` and the separate worktree branch `worktree-phase0-plan-capture`, a different session executing Phase 0, diverged from common ancestor `72ed960`, and both carry LEDGER.md edits. One divergence point, one merge, planned below.

**LEDGER.md staleness on two entries, RECONCILED 2026-08-04, kept for the record.** Its ISSUES section used to show both "Q6 clarification needs owner ratification" and "Active-state limit inflation / ratchet" as `Status: BLOCKED on owner` while both had already moved past that point in this document. Reconciled after the freeze lifted: the Q6 clarification and the ratchet build decision are now recorded in LEDGER DECISIONS as FINAL, each sourced to the owner's 2026-07-30 ruling, and the one surviving ISSUES entry is narrowed to the corrected ratchet mechanism, which remains UNRULED (above) and is authority for nothing until ratified. The two files now agree; this document is no longer ahead of the ledger on either item.

**Merge plan for `worktree-phase0-plan-capture` into `main`, RUN 2026-08-03, merge commit `ce6964d`.** Executed per the mechanics below, which stand as the permanent merge procedure for this project. **The Phase-0-green condition this plan originally carried is SUPERSEDED** by the owner ruling of 2026-08-03 (LEDGER DECISIONS, FINAL): a branch merges when the work in that branch is complete and proven, not when the phase as a whole is green, with the owner-instruction requirement unchanged. That ruling changed this plan; it was not a violation of it, and `ce6964d` is the new standard applied rather than an exception to the old one. No phase-green gate survives anywhere in this document.

Mechanics, unchanged and still binding for every future merge: a real merge commit, never rebase, never fast-forward-only, never `-X ours`/`-X theirs`. Resolve every LEDGER.md conflict by hand, keeping both sides' entries (union, never choosing a side), respecting each section's own ordering convention (ACTIONS newest-first, DECISIONS chronological, ISSUES priority-ordered). After merging, verify by counting entries per section before and after, and by identity-matching each entry's opening line against the merged file, not count alone, so a balanced count cannot mask one entry silently dropped and an unrelated one double-counted.

**The LEDGER.md write freeze above is LIFTED** by that merge, which is the reconciliation the freeze was waiting for.
