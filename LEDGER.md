...
Issue:  R1, crash-loop primitive, HELD by the owner pending the Stage 1 measurement of routine-shutdown behavior. Measurement delivered: a routine Windows shutdown runs OnDeinit and marks its session CLEAN (halt record 5 of the overnight event, corroborated by the zeroed mutex heartbeat and the absent takeover line; the deinit journal lines themselves were lost unflushed, so the halt file, not the journal, is the reliable shutdown witness). R1 re-presented with a fourth option costed in the contact 1 report. R2 stays pending R1.
Action: Await the owner's R1 ruling. Blocked behind it: A3, A4, A6.
Status: BLOCKED on owner
